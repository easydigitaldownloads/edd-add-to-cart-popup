...
<div class="edd-acp-popup" style="<?php echo esc_attr($style); ?>">
	...
	<p class="edd-acp-popup-singular">
		<?php
		$singularText = apply_filters('edd_acp_popup_singular_text', $settings->getSubValue('maintext'), $viewbag->downloadId, $settings);
		$singularTextFormatted = sprintf($singularText, '<strong class="item-name"></strong>');
		echo apply_filters('edd_acp_popup_singular_text_formatted', $singularTextFormatted, $viewbag->downloadId, $settings);
		?>
	</p>
	<p class="edd-acp-popup-plural">
		<?php
		$pluralText = apply_filters('edd_acp_popup_plural_text', $settings->getSubValue('pluraltext'), $viewbag->downloadId, $settings);
	...
	<p>
		<?php
		$checkoutButtonText = apply_filters('edd_acp_popup_checkout_button_text', $settings->getSubValue('checkoutBtnText'), $viewbag->downloadId, $settings); 
		$continueButtonText = apply_filters('edd_acp_popup_continue_button_text', $settings->getSubValue('continueBtnText'), $viewbag->downloadId, $settings);
		?>
